...
- `[x]` **演示代码预填**：编辑器首次打开即显示完整 `MaCrossStrategy`（含 IntParameter/CategoricalParameter）
- `[x]` **演示按钮**：顶部工具栏橙绿渐变大按钮"✨ 运行演示策略 (双均线交叉)"，idle 状态持续呼吸光晕动画
- `[x]` **一键运行**：点击后自动 save → submit（时间范围 2025-09 ~ 2026-01，futures 4h），无需用户手动操作
- `[x]` **结果接通底部面板**：回测日志实时流入"回测控制台" Tab，完成后摘要卡片+CSV 下载全部就位

## 🔄 阶段 10：交互体验纠偏 (对标 TV 原生挂载流)

> **目标**：废弃“让普通用户看代码”的逻辑，将策略选用入口直接集成到主图表页。

- `[x]` **修复主图表页空载**
  - `[x]` 将渲染条件从 `loading ?` 改为 `candles.length === 0 && loading`，第一批 K 线数据到达即立刻渲染，翻页加载不再阻断图表显示
- `[x]` **图表页顶栏下拉组件**
  - `[x]` 时间周期选择器旁新增 `ƒx 策略` 下拉按钮，预置 `MA 双均线交叉`，激活时高亮显示策略名
- `[x]` **静默调用与联动**
  - `[x]` `handleApplyStrategy()`：静默 save 模板 → POST 回测 → 3s 轮询 → 完成后 `setMarkers` 画买卖箭头
  - `[x]` `ChartPanel` 新增 `markers` prop，通过 `candleSeries.setMarkers()` 渲染绿色买入 ↑ / 红色卖出 ↓ 箭头
  - `[x]` 回测完成后自动填充底部 `StrategyTesterPanel`（日志/摘要卡片/交易明细/CSV 下载）
