...
function renderTrades(){
  ...
  for(let i=0;i<12;i++){
    ...
    const days=Math.floor(r()*5+1);
    const pnlAmt=(exit-entry)*0.1;
    rows.push({n:i+1,date:`25/${mo}/15`,side:r()>.3?'LONG':'SHORT',
      entry,exit,pnl:pct*100,days,pnlAmt});
  }
  tbody.innerHTML=rows.map(t=>`<tr>
    ...
    <td style="color:var(--text2)">${t.days}天</td>
    <td class="${t.pnl>=0?'up':'dn'}">${t.pnl>=0?'+':''}${t.pnl.toFixed(2)}%</td>
    <td class="${t.pnlAmt>=0?'up':'dn'}">${t.pnlAmt>=0?'+':''}$${Math.abs(t.pnlAmt).toFixed(0)}</td>
  </tr>`).join('');
}
...
function renderScoreBreakdown(){
  ...
  if(!el)return;
  const items=[['卡玛比率',87],['索提诺比率',82],['盈利因子',91],['夏普比率',78],['净收益',95]];
...
/* ======= S4 TAB SWITCH ======= */
function switchS4Tab(idx){
  const tabs=document.querySelectorAll('#s4tabs .ptab');
  tabs.forEach((t,i)=>t.classList.toggle('on',i===idx));
  [0,1].forEach(i=>{
    const el=document.getElementById('s4t'+i);
    if(el)el.style.display=i===idx?'':'none';
  });
}

/* ======= S3 TAB SWITCH ======= */
function switchS3Tab(idx){
  const tabs=document.querySelectorAll('#s3tabs .ptab');
  tabs.forEach((t,i)=>t.classList.toggle('on',i===idx));
  [0,1,2,3].forEach(i=>{
    const el=document.getElementById('s3t'+i);
    if(el)el.style.display=i===idx?'':'none';
  });
  // draw equity when tab 1 opens
  if(idx===1)drawEquity();
}
...
function downloadCSV(type){
  ...
  if(type==='trades'){
    ...
    for(let i=1;i<=12;i++){
      ...
      const exit=entry*(1+pct);
      price=exit;
      rows.push(`${i},2025/${String(i).padStart(2,'0')}/15,${r()>.3?'LONG':'SHORT'},${entry.toFixed(2)},${exit.toFixed(2)},${(pct*100).toFixed(2)}%`);
    }
  } else if(type==='equity'){
    filename='btc_equity.csv';
    rows=['日期,净值'];
    const eq=genEquity();
    eq.forEach((v,i)=>{
      const d=new Date(2024,0,1+i);
      rows.push(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')},${v.toFixed(2)}`);
    });
  } else if(type==='opt_all'){
    filename='btc_optimize_all.csv';
    const d=genScatter();
    rows=['短期窗口,长期窗口,收益率%,最大回撤%,夏普比率,交易数'];
    d.forEach(x=>rows.push(`${x.s},${x.l},${x.ret.toFixed(2)},${x.dd.toFixed(2)},${x.sharpe.toFixed(3)},${x.trades}`));
  } else if(type==='opt_top'){
    filename='btc_optimize_top10.csv';
    const d=genScatter();d.sort((a,b)=>(b.ret/b.dd*b.sharpe)-(a.ret/a.dd*a.sharpe));
    rows=['排名,短期窗口,长期窗口,收益率%,最大回撤%,夏普比率,交易数'];
    d.slice(0,10).forEach((x,i)=>rows.push(`${i+1},${x.s},${x.l},${x.ret.toFixed(2)},${x.dd.toFixed(2)},${x.sharpe.toFixed(3)},${x.trades}`));
  } else {
    filename='btc_tradingview.csv';
    rows=['Trade #,Type,Date/Time,Price,Contracts,Profit,Profit %,Cum. Profit,Run-up,Run-up %,Drawdown,Drawdown %'];
    let price=88000;
    for(let i=1;i<=12;i++){
      const entry=price*(1+(r()-.5)*.04);
      const pct=(r()-.36)*.14;
      const exit=entry*(1+pct);
      price=exit;
      const profit=(exit-entry).toFixed(2);
      rows.push(`${i},Long Entry,2025-${String(i).padStart(2,'0')}-15 00:00,${entry.toFixed(2)},1,${profit},${(pct*100).toFixed(2)}%,${profit},${Math.abs(profit*1.2).toFixed(2)},${(pct*120).toFixed(2)}%,${Math.abs(profit*.3).toFixed(2)},${(pct*30).toFixed(2)}%`);
    }
  }
...
function saveStrategy(){
  ...
  if(!btn)return;
  // 读取当前策略名
  const name=document.querySelector('.tmpl-item.on')?.textContent?.trim()||'未命名策略';
  // 从 localStorage 读已有列表
  const saved=JSON.parse(localStorage.getItem('btc_saved_strategies')||'[]');
  const entry={name,code:'MA双均线交叉',ts:new Date().toLocaleString('zh')};
  // 去重 + 追加
  const exists=saved.findIndex(s=>s.name===name);
  if(exists>=0)saved[exists]=entry;else saved.push(entry);
  localStorage.setItem('btc_saved_strategies',JSON.stringify(saved));
  // 按钮反馈
  btn.textContent='✓ 已保存';btn.style.color='#26a69a';btn.style.borderColor='rgba(38,166,154,.4)';
  setTimeout(()=>{btn.textContent='保存策略';btn.style.color='';btn.style.borderColor='';},2000);
}
...
function runSim(){
  ...
  const lines=[
    ...
    '<span style="color:var(--bull)">净收益: +496.97%  |  最大回撤: -68.74%  |  夏普比率: 0.78</span>',
  ];
