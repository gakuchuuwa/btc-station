    ...
    <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-mute)', lineHeight: 1.7 }}>
          上传 TradingView 策略测试器导出的"交易清单"（xlsx），或 MetaTrader5 策略测试报告（html），
      ...
      >
        ...
        <div style={{ fontSize: 12, color: 'var(--text-mute)' }}>
          支持 .xlsx（多 sheet 自动选"交易清单"） / MT5 报告 .html
        </div>
        <input
          ref={inputRef} type="file" accept=".xlsx,.xls,.html,.htm"
          style={{ display: 'none' }}
      ...
      {data && (
        <>
          ...
          <div style={{ marginTop: 16, fontSize: 11, color: 'var(--text-dim)', lineHeight: 1.7 }}>
            ...
            Scale-in 加仓单（P1 信号持仓中触发、仓位为 P1 的一半）计入 P1 形态统计；
