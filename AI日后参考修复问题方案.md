...
### 1. 以 README 当前架构为准，不要凭旧代码扩展

产品页面、导航、目录会随版本演进变化。新增主功能或修改导航前，**先读 README 顶部的产品架构说明**，以那里为准。不要看到代码里有某个文件夹就假定它是当前主线——可能是历史遗留。

发现疑似旧目录（如 `analysis`、`chart`、`backtest`、`hyperopt`、`live` 等）时，核对 README 是否还把它们列为主页面；不在 README 主架构里的，视为历史代码，**不要继续围绕它们加功能**。
...
回测和稳健性分析是两个不同性质的功能，必须分页面承载，不要为了"方便"塞到同一页。

- **回测**回答：这个策略和这组参数在历史上跑出来什么结果？
- **稳健性分析**回答：这组参数是真的好，还是只是过拟合孤峰？

具体由哪个页面承载，以 README 当前架构为准。
